    ...
    <Document>
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.section}>
          ...
          <Field label="ชื่องาน:" value={formData.work_name || ''} />
          <View style={styles.headerInfoRow}>
            <Field label="อนุมัติเลขที่:" value={formData.approval_number || ''} style={{width: '48%'}} />
            <Field label="ลงวันที่:" value={formData.approval || ''} style={{width: '48%'}}/>
          </View>
          <View style={styles.headerInfoRow}>
            <Field label="หมายเลขงาน:" value={formData.work_id || ''} style={{width: '48%'}} />
            <Field label="วันที่ดำเนินการตรวจ:" value={formData.inspection_date || ''} style={{width: '48%'}} />
          </View>
          
          <View style={{...styles.row, marginTop: 5}}>
            <Checkbox checked={formData.operation} label="กฟภ. ดำเนินการ" />
            <Checkbox checked={formData.contractor_work} label="งานจ้างฯบริษัท" />
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.headerInfoRow}>
            <Field label="ปริมาณงาน (วงจร-กม.):" value={formData.hv_work_volume || ''} style={{width: '48%'}} />
            <Field label="จำนวนเสา (ต้น):" value={formData.hv_work_volume_poles || ''} style={{width: '48%'}} />
          </View>
          <View style={styles.headerInfoRow}>
            <Field label="รับไฟจากสถานี:" value={formData.hv_station || ''} style={{width: '48%'}} />
            <Field label="ฟีดเดอร์:" value={formData.hv_feeder || ''} style={{width: '48%'}} />
          </View>
          <View style={styles.headerInfoRow}>
            <Field label="เฟสที่ต่อ:" value={formData.hv_phase || ''} style={{width: '48%'}} />
            <Field label="หม้อแปลงรวม (KVA):" value={formData.hv_transformer_kva || ''} style={{width: '48%'}} />
          </View>
          
          <Text style={{fontWeight: 'bold', marginBottom: 2, marginTop: 8}}>งานแรงต่ำ</Text>
          <View style={styles.headerInfoRow}>
            <Field label="ปริมาณงาน (วงจร-กม.):" value={formData.lv_work_volume || ''} style={{width: '48%'}} />
            <Field label="จำนวนเสา (ต้น):" value={formData.lv_work_volume_poles || ''} style={{width: '48%'}} />
          </View>
          
          <Text style={{fontWeight: 'bold', marginBottom: 2, marginTop: 8}}>ผู้ควบคุมงาน</Text>
          <Field label="ชื่อ:" value={formData.supervisor_name || ''} />
          <View style={styles.headerInfoRow}>
            <Field label="ตำแหน่ง:" value={formData.supervisor_position || ''} style={{width: '48%'}} />
            <Field label="สังกัด:" value={formData.supervisor_affiliation || ''} style={{width: '48%'}} />
          </View>
        </View>

        <View style={styles.section}>
          <Text style={styles.sectionTitle}>รายการตรวจสอบ</Text>
          {formData.inspection_items && formData.inspection_items.map((category, index) => (
      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.section}>
          ...
          <View style={{...styles.row, paddingLeft: 10}}>
            <Checkbox checked={formData.inspection_result === 'ถูกต้องตามมาตรฐาน กฟภ.'} label="ตรวจสอบแล้ว ถูกต้องตามมาตรฐาน กฟภ." />
            <Checkbox checked={formData.inspection_result === 'เห็นควรแก้ไข'} label="ตรวจสอบแล้ว เห็นควรแก้ไขให้ถูกต้องตามรายการข้างต้น" />
          </View>
        </View>

        <View style={styles.signatureSection}>
          <View style={styles.signatureBox}>
            {formData.user_signature && <Image style={styles.signatureImage} src={formData.user_signature} alt="" />}
            <Text style={styles.signatureLine}>(..................................................)</Text>
            <Text style={{fontSize: 8}}>ผู้ขอใช้ไฟฟ้าหรือผู้แทน</Text>
            <Text style={{fontSize: 8}}>ตำแหน่ง: {formData.user_position || ''}</Text>
          </View>
          <View style={styles.signatureBox}>
            {formData.inspector_signature && <Image style={styles.signatureImage} src={formData.inspector_signature} alt="" />}
            <Text style={styles.signatureLine}>(..................................................)</Text>
