  ...
  const handleSubmit = async (e, refs = {}) => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        // ตรวจสอบว่า uploadData มีค่าก่อนเรียก getPublicUrl
        if (uploadData && uploadData.path) {
          ...
          console.warn("Upload data is undefined. Skipping public URL retrieval.");
        }
      }
      ...
      delete dataToSubmit.created_at; 
        dbOperation = supabase.from(tableName).update(dataToSubmit).eq('id', id).select(columnsToSelect).single();
