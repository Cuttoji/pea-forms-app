...
export function useFormManager(tableName, initialData, imageBucketName = 'default') {
  ...
  const handleSubmit = async (e, refs = {}) => {
    ...
    try {
      ...
      const numericFields = [
        'latitude', 'longitude', 'estimatedLoad', 'cableSizeSqmm',
        'breakerAmpRating', 'groundWireSizeSqmm', 'groundResistanceOhm'
      ];
      ...
      let dbOperation;
      delete dataToSubmit.created_at; // ลบ field ที่ DB จัดการเองเสมอ
      ...
      if (id) {
        ...
        delete dataToSubmit.id; // ลบ id ที่เป็น null ออกก่อน insert
      ...
      if (!id) {
        ...
        for (const key in resultData) {
            if (Object.prototype.hasOwnProperty.call(sanitizedResult, key)) {
