...
export const useFormManager = (tableName, initialFormData, arrayFields = [], selectFields = '*', bucketName = 'form-images') => {
  ...
  try {
    ...
    let dataToSubmit = transformFormData(formData, user.id, !!id);
    
    let result;
    if (id) {
      // สำหรับ update
      const { id: formId, ...updateData } = dataToSubmit;
      ...
      // สำหรับ insert - ลบ id ออกแน่นอน
      const { id: formId, ...insertData } = dataToSubmit;
      result = await supabaseRef.current
        .from(tableName)
        .insert([insertData])
        .select();
